...
#ifndef _CORECRYPTO_CCBFV_CIPHER_PLAIN_CTX_H
#define _CORECRYPTO_CCBFV_CIPHER_PLAIN_CTX_H
...
#define CCBFV_CIPHER_PLAIN_CTX_DELTA(cipher_plain_ctx) ((cipher_plain_ctx)->data)

/// @brief Get the delta array where i'th entry stores `floor(q / t) % q_i`, where `t` is the plaintext
/// modulus. Also called `delta` in the literature. Each element has `CCPOLYZP_PO2CYC_NUNITS_PER_COEFF` units.
/// @param cipher_plain_ctx The cipher-plain context to get the delta array from
#define CCBFV_CIPHER_PLAIN_CTX_DELTA_CONST(cipher_plain_ctx) ((const cc_unit *)((cipher_plain_ctx)->data))
...
#define CCBFV_CIPHER_PLAIN_CTX_PLAIN_INCREMENT(cipher_plain_ctx) \
    ((cipher_plain_ctx)->data + (cipher_plain_ctx)->cipher_ctx->dims.nmoduli * CCPOLYZP_PO2CYC_NUNITS_PER_COEFF)

/// @brief Get the plain increment array
/// @param cipher_plain_ctx The cipher-plain context to get the plain increment array from
/// @details Plain increment array stores `q_i - t`.
#define CCBFV_CIPHER_PLAIN_CTX_PLAIN_INCREMENT_CONST(cipher_plain_ctx) \
    ((cipher_plain_ctx)->data + (cipher_plain_ctx)->cipher_ctx->dims.nmoduli * CCPOLYZP_PO2CYC_NUNITS_PER_COEFF)
...
CC_WARN_RESULT CC_NONNULL_ALL int ccbfv_cipher_plain_ctx_init_ws(cc_ws_t ws,
                                                                 ccbfv_cipher_plain_ctx_t cipher_plain_ctx,
                                                                 ccbfv_param_ctx_const_t param_ctx,
                                                                 ccpolyzp_po2cyc_ctx_const_t cipher_ctx);

#endif /* _CORECRYPTO_CCBFV_CIPHER_PLAIN_CTX_H */
