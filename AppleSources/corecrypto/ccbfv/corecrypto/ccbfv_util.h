...
#ifndef _CORECRYPTO_CCFBV_UTIL_H_
#define _CORECRYPTO_CCFBV_UTIL_H_
...
CC_INLINE CC_NONNULL_ALL void ccbfv_ciphertext_coeff_init(ccbfv_ciphertext_coeff_t ctext,
                                                          ccbfv_param_ctx_const_t param_ctx,
                                                          uint32_t npolys,
                                                          ccpolyzp_po2cyc_ctx_const_t cipher_ctx)
{
    ccbfv_ciphertext_coeff_t ctext_coeff = (ccbfv_ciphertext_coeff_t)ctext;
    ctext_coeff->param_ctx = param_ctx;
    ctext_coeff->npolys = npolys;
    // note we cannot use `ccbfv_ciphertext_coeff_polynomial(ctext, 0)` here, because the context is not initialized yet
    ...
    for (uint32_t poly_idx = 1; poly_idx < npolys; ++poly_idx) {
        ccpolyzp_po2cyc_coeff_t poly = ccbfv_ciphertext_coeff_polynomial(ctext_coeff, poly_idx);
...
/// @details This makes sure that the ciphertext and the polynomial contexts are initialized, it does not however
/// initialize the actual polynomial coefficients.
CC_INLINE CC_NONNULL_ALL void ccbfv_ciphertext_eval_init(ccbfv_ciphertext_eval_t ctext,
                                                         ccbfv_param_ctx_const_t param_ctx,
                                                         uint32_t npolys,
                                                         ccpolyzp_po2cyc_ctx_const_t cipher_ctx)
{
    ccbfv_ciphertext_coeff_init((ccbfv_ciphertext_coeff_t)ctext, param_ctx, npolys, cipher_ctx);
}

/// @brief Initializes the plaintext with the given parameter context
/// @param ptext The plaintext; should be allocated via CCBFV_PLAINTEXT_ALLOC_WS
/// @param param_ctx The parameter context with which to initialize the plaintext
CC_INLINE CC_NONNULL_ALL void ccbfv_plaintext_init(ccbfv_plaintext_t ptext, ccbfv_param_ctx_const_t param_ctx)
{
    ccpolyzp_po2cyc_ctx_const_t plain_ctx = ccbfv_param_ctx_plaintext_context(param_ctx);
    ptext->param_ctx = param_ctx;
...
CC_NONNULL_ALL void ccbfv_mul_poly_sk(ccpolyzp_po2cyc_eval_t r, ccpolyzp_po2cyc_eval_const_t x, ccbfv_secret_key_const_t sk);

#endif /* _CORECRYPTO_CCFBV_UTIL_H_ */
