...
#ifndef _CORECRYPTO_CCBFV_GALOIS_H_
#define _CORECRYPTO_CCBFV_GALOIS_H_
...
#define CCBFV_GALOIS_KEY_GALOIS_ELTS(galois_key) ((uint32_t *)(galois_key)->data)
#define CCBFV_GALOIS_KEY_GALOIS_ELTS_CONST(galois_key) ((const uint32_t *)(galois_key)->data)

/// @brief Returns the ciphertexts in the Galois key
/// @param galois_key The Galois key
#define CCBFV_GALOIS_KEY_CIPHERS(galois_key)                                           \
    ((ccbfv_ciphertext_eval_t)(((cc_unit *)CCBFV_GALOIS_KEY_GALOIS_ELTS(galois_key)) + \
                               ccn_nof_size(sizeof(uint32_t) * galois_key->ngalois_elts)))
#define CCBFV_GALOIS_KEY_CIPHERS_CONST(galois_key)                                                       \
    ((ccbfv_ciphertext_eval_const_t)(((const cc_unit *)CCBFV_GALOIS_KEY_GALOIS_ELTS_CONST(galois_key)) + \
                                     ccn_nof_size(sizeof(uint32_t) * galois_key->ngalois_elts)))
...
CC_WARN_RESULT CC_NONNULL_ALL cc_size ccbfv_galois_key_nof_n(ccbfv_param_ctx_const_t param_ctx, uint32_t ngalois_elts);

/// @brief Allocates memory for a ccbfv_galois_key_t
...
#define CCBFV_GALOIS_KEY_ALLOC_WS(ws, param_ctx, ngalois_elts) \
    (ccbfv_galois_key_t) CC_ALLOC_WS(ws, ccbfv_galois_key_nof_n((param_ctx), (ngalois_elts)))
...
CC_WARN_RESULT CC_NONNULL_ALL ccbfv_ciphertext_eval_t ccbfv_galois_key_ciphertext(ccbfv_galois_key_t galois_key,
...
/// @details The returned ciphertext is `([q_ks * \tilde{P}_{Q_i}(s_A) - a * s_B]_{q_ks * Q_i} + e, a)_{[Q_i, q_sk]}` where
/// `i = cipher_idx` and `s_A = s(x^{galois_elt})`, for secret key polynomial `s(x)`
CC_WARN_RESULT CC_NONNULL_ALL ccbfv_ciphertext_eval_const_t ccbfv_galois_key_ciphertext_const(ccbfv_galois_key_const_t galois_key,
                                                                                              uint32_t galois_elt_idx,
                                                                                              uint32_t cipher_idx);
...
/// `\tilde{P})_{Q_i}(s_A)_j = [s_A * (Q_i / \tilde{Q}_j) * (Q_i / \tilde{Q_j})^{-1}]_{Q_i}` and
/// `\tilde{Q_j} = q_j`.
CC_WARN_RESULT CC_NONNULL((1, 2, 4, 5, 6)) int ccbfv_galois_key_generate_ws(cc_ws_t ws,
...
ccbfv_galois_key_save_ws(cc_ws_t ws, uint32_t nbytes_poly0s, uint8_t *poly0s, ccbfv_galois_key_const_t galois_key);
...
/// @return CCERR_OK if successful
CC_WARN_RESULT CC_NONNULL_ALL int ccbfv_galois_key_load_ws(cc_ws_t ws,
                                                           ccbfv_galois_key_t galois_key,
                                                           uint32_t ngalois_elts,
...
CC_WARN_RESULT CC_NONNULL((3)) bool ccbfv_galois_key_find_galois_elt(uint32_t *galois_elt_idx,
                                                                     uint32_t galois_elt,
...
/// @return CCERR_OK if successful
CC_NONNULL_ALL CC_WARN_RESULT int ccbfv_ciphertext_apply_galois_ws(cc_ws_t ws,
...
CC_WARN_RESULT CC_NONNULL_ALL int ccbfv_ciphertext_galois_key_switch_ws(cc_ws_t ws,
                                                                        ccbfv_ciphertext_coeff_t r,
                                                                        uint32_t galois_elt,
                                                                        ccbfv_galois_key_const_t galois_key);
...
CC_WARN_RESULT CC_NONNULL_ALL int ccbfv_ciphertext_rotate_rows_left_ws(cc_ws_t ws,
                                                                       ccbfv_ciphertext_coeff_t r,
                                                                       ccbfv_ciphertext_coeff_const_t ctext,
                                                                       uint32_t step,
                                                                       ccbfv_galois_key_const_t galois_key);
...
/// @return CCERR_OK if successful
/// @details Not constant time; the step is considered public
CC_WARN_RESULT CC_NONNULL_ALL int ccbfv_ciphertext_rotate_rows_right_ws(cc_ws_t ws,
                                                                        ccbfv_ciphertext_coeff_t r,
                                                                        ccbfv_ciphertext_coeff_const_t ctext,
                                                                        uint32_t step,
                                                                        ccbfv_galois_key_const_t galois_key);
...
/// @param galois_key The Galois key; must contain the Galois element for the step
/// @return CCERR_OK if successful
/// @details Not constant time. In BFV, the plaintext can be viewed as a 2 x (N/2) matrix. This function rotates the
/// elements in each column. Since there are only two elements in each
/// column, this simply swaps the two rows.
CC_WARN_RESULT CC_NONNULL_ALL int ccbfv_ciphertext_swap_columns_ws(cc_ws_t ws,
                                                                   ccbfv_ciphertext_coeff_t r,
                                                                   ccbfv_ciphertext_coeff_const_t ctext,
                                                                   ccbfv_galois_key_const_t galois_key);

#endif /* _CORECRYPTO_CCBFV_GALOIS_H_ */
