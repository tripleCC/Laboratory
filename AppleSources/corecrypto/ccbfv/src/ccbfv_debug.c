...
void ccbfv_galois_key_lprint(ccbfv_galois_key_const_t galois_key, const char *cc_cstring label)
{
    ...
    for (uint32_t galois_elt_idx = 0; galois_elt_idx < galois_key->ngalois_elts; ++galois_elt_idx) {
        ...
        uint32_t nmoduli = ccbfv_param_ctx_encrypt_key_context(galois_key->param_ctx)->dims.nmoduli - 1;

        for (uint32_t cipher_idx = 0; cipher_idx < nmoduli; ++cipher_idx) {
            ccbfv_ciphertext_eval_const_t ctext = ccbfv_galois_key_ciphertext_const(galois_key, galois_elt_idx, cipher_idx);
            ccbfv_ciphertext_eval_lprint(ctext, NULL);
        }
        cc_printf("}");
    }
    cc_printf("}\n");
...
void ccbfv_relin_key_lprint(ccbfv_relin_key_const_t relin_key, const char *cc_cstring label)
{
    ...
    uint32_t nmoduli = ccbfv_param_ctx_encrypt_key_context(relin_key->param_ctx)->dims.nmoduli - 1;
    cc_printf("nmoduli %" PRIu32 "\n", nmoduli);
    for (uint32_t cipher_idx = 0; cipher_idx < nmoduli; ++cipher_idx) {
        cc_printf("cipher_idx %" PRIu32 "\n", cipher_idx);
        ccbfv_ciphertext_eval_const_t ctext = ccbfv_relin_key_ciphertext_const(relin_key, cipher_idx);
        ccbfv_ciphertext_eval_lprint(ctext, NULL);
    }
    cc_printf("}");
    cc_printf("}\n");
