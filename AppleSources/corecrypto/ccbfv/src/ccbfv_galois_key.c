...
cc_size ccbfv_galois_key_nof_n(ccbfv_param_ctx_const_t param_ctx, uint32_t ngalois_elts)
{
    cc_size rv = ccn_nof_size(sizeof_struct_ccbfv_galois_key());
    ...
    rv += ngalois_elts * nciphers * ccbfv_ciphertext_nof_n(&dims, ccbfv_ciphertext_fresh_npolys());
    return rv;
}

ccbfv_ciphertext_eval_t ccbfv_galois_key_ciphertext(ccbfv_galois_key_t galois_key, uint32_t galois_elt_idx, uint32_t cipher_idx)
{
    cc_assert(galois_elt_idx < galois_key->ngalois_elts);
    cc_unit *galois_cipher = (cc_unit *)CCBFV_GALOIS_KEY_CIPHERS(galois_key);
    ccpolyzp_po2cyc_ctx_const_t galois_key_ctx = ccbfv_param_ctx_encrypt_key_context(galois_key->param_ctx);
    cc_assert(cipher_idx < galois_key_ctx->dims.nmoduli);
    uint32_t nciphers = CC_MAX_EVAL(1, galois_key_ctx->dims.nmoduli - 1);

    galois_cipher += galois_elt_idx * nciphers * ccbfv_ciphertext_nof_n(&galois_key_ctx->dims, ccbfv_ciphertext_fresh_npolys());
    galois_cipher += cipher_idx * ccbfv_ciphertext_nof_n(&galois_key_ctx->dims, ccbfv_ciphertext_fresh_npolys());
    return (ccbfv_ciphertext_eval_t)galois_cipher;
}

ccbfv_ciphertext_eval_const_t
ccbfv_galois_key_ciphertext_const(ccbfv_galois_key_const_t galois_key, uint32_t galois_elt_idx, uint32_t cipher_idx)
{
    cc_assert(galois_elt_idx < galois_key->ngalois_elts);
    const cc_unit *galois_cipher = (const cc_unit *)CCBFV_GALOIS_KEY_CIPHERS_CONST(galois_key);
    ccpolyzp_po2cyc_ctx_const_t galois_key_ctx = ccbfv_param_ctx_encrypt_key_context(galois_key->param_ctx);
    cc_assert(cipher_idx < galois_key_ctx->dims.nmoduli);
    uint32_t nciphers = CC_MAX_EVAL(1, galois_key_ctx->dims.nmoduli - 1);
    ...
    galois_cipher += galois_elt_idx * nciphers * ccbfv_ciphertext_nof_n(&galois_key_ctx->dims, ccbfv_ciphertext_fresh_npolys());
    galois_cipher += cipher_idx * ccbfv_ciphertext_nof_n(&galois_key_ctx->dims, ccbfv_ciphertext_fresh_npolys());
    return (ccbfv_ciphertext_eval_const_t)galois_cipher;
}

cc_size CCBFV_GALOIS_KEY_GENERATE_SINGLE_WORKSPACE_N(cc_size degree, cc_size nmoduli)
{
...
CC_WARN_RESULT CC_NONNULL((1, 2, 5, 6)) static int ccbfv_galois_key_generate_single_ws(cc_ws_t ws,
                                                                                       ccbfv_galois_key_t galois_key,
                                                                                       uint32_t galois_elt,
                                                                                       ...
                                                                                       struct ccrng_state *rng)
{
    ...
    uint32_t *galois_key_elts = CCBFV_GALOIS_KEY_GALOIS_ELTS(galois_key);
    ...
    for (uint32_t rns_idx = 0; rns_idx < nciphers; ++rns_idx) {
        ...
        ccbfv_ciphertext_eval_t galois_cipher = ccbfv_galois_key_ciphertext(galois_key, galois_elt_idx, rns_idx);
        ...
        ccpolyzp_po2cyc_eval_t gk_poly0 = ccbfv_ciphertext_eval_polynomial(galois_cipher, 0);
...
cc_size CCBFV_GALOIS_KEY_GENERATE_WORKSPACE_N(cc_size degree, cc_size nmoduli)
{
    return CCBFV_GALOIS_KEY_GENERATE_SINGLE_WORKSPACE_N(degree, nmoduli);
}

int ccbfv_galois_key_generate_ws(cc_ws_t ws,
                                 ccbfv_galois_key_t galois_key,
                                 ...
                                 struct ccrng_state *rng)
{
    ...
    cc_require_or_return(nseeds == 0 || nseeds == nciphers * ngalois_elts * ccbfv_rng_seed_sizeof(), CCERR_PARAMETER);
    ...
    for (uint32_t galois_elt_idx = 0; galois_elt_idx < ngalois_elts; ++galois_elt_idx) {
        uint8_t *single_seeds = seeds ? seeds + galois_elt_idx * nciphers * ccbfv_rng_seed_sizeof() : NULL;
        rv = ccbfv_galois_key_generate_single_ws(
...
int ccbfv_galois_key_generate(ccbfv_galois_key_t galois_key,
                              uint32_t ngalois_elts,
                              ...
                              struct ccrng_state *rng)
{
    ...
    uint32_t nmoduli = ccbfv_param_ctx_key_ctx_nmoduli(param_ctx);
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCBFV_GALOIS_KEY_GENERATE_WORKSPACE_N(ccbfv_param_ctx_polynomial_degree(param_ctx), nmoduli));
    int rv = ccbfv_galois_key_generate_ws(ws, galois_key, ngalois_elts, galois_elts, secret_key, param_ctx, nseeds, seeds, rng);
    CC_FREE_WORKSPACE(ws);
    return rv;
}

int ccbfv_galois_key_save_ws(cc_ws_t ws, uint32_t nbytes_poly0s, uint8_t *poly0s, ccbfv_galois_key_const_t galois_key)
{
    int rv = CCERR_OK;
    ccpolyzp_po2cyc_ctx_const_t key_ctx = ccbfv_param_ctx_encrypt_key_context(galois_key->param_ctx);
    ...
    for (uint32_t galois_elt_idx = 0; galois_elt_idx < ngalois_elts; ++galois_elt_idx) {
        for (uint32_t cipher_idx = 0; cipher_idx < nciphers; ++cipher_idx) {
            ccbfv_ciphertext_eval_const_t ciphertext = ccbfv_galois_key_ciphertext_const(galois_key, galois_elt_idx, cipher_idx);
...
int ccbfv_galois_key_save(uint32_t nbytes_poly0s, uint8_t *poly0s, ccbfv_galois_key_const_t galois_key)
{
    CC_ENSURE_DIT_ENABLED

    CC_DECL_WORKSPACE_OR_FAIL(ws, CCBFV_GALOIS_KEY_SAVE_WORKSPACE_N(ccbfv_param_ctx_polynomial_degree(galois_key->param_ctx)));
    int rv = ccbfv_galois_key_save_ws(ws, nbytes_poly0s, poly0s, galois_key);
    CC_FREE_WORKSPACE(ws);
    return rv;
}
...
/// @return CCERR_OK if Galois key was generated successfully
CC_WARN_RESULT CC_NONNULL((1, 2, 5, 6)) static int ccbfv_galois_key_load_single_ws(cc_ws_t ws,
                                                                                   ccbfv_galois_key_t galois_key,
                                                                                   uint32_t galois_elt,
                                                                                   uint32_t galois_elt_idx,
                                                                                   ccbfv_param_ctx_const_t param_ctx,
                                                                                   ...
                                                                                   const uint8_t *seeds)
{
    ...
    for (uint32_t rns_idx = 0; rns_idx < nciphers; ++rns_idx) {
        ccbfv_ciphertext_eval_t galois_cipher = ccbfv_galois_key_ciphertext(galois_key, galois_elt_idx, rns_idx);
        ccbfv_ciphertext_eval_init(galois_cipher, param_ctx, ccbfv_ciphertext_fresh_npolys(), key_ctx);
        rv = ccbfv_deserialize_seeded_ciphertext_eval_ws(
            ws, galois_cipher, nbytes_per_poly, poly0s, (ccbfv_rng_seed_const_t)seeds);
        cc_require(rv == CCERR_OK, errOut);
        poly0s += nbytes_per_poly;
...
int ccbfv_galois_key_load_ws(cc_ws_t ws,
                             ccbfv_galois_key_t galois_key,
                             uint32_t ngalois_elts,
                             const uint32_t *galois_elts,
                             ccbfv_param_ctx_const_t param_ctx,
                             ...
                             const uint8_t *cc_counted_by(nbytes_seeds) seeds)
{
    ...
    for (uint32_t galois_elt_idx = 0; galois_elt_idx < ngalois_elts; ++galois_elt_idx) {
        ...
        rv = ccbfv_galois_key_load_single_ws(
...
int ccbfv_galois_key_load(ccbfv_galois_key_t galois_key,
                          uint32_t ngalois_elts,
                          const uint32_t *galois_elts,
                          ccbfv_param_ctx_const_t param_ctx,
                          uint32_t nbytes_poly0s,
                          ...
                          const uint8_t *cc_counted_by(nbytes_seeds) seeds)
{
    ...
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCBFV_GALOIS_KEY_LOAD_WORKSPACE_N(ccbfv_param_ctx_polynomial_degree(param_ctx)));
    int rv = ccbfv_galois_key_load_ws(
        ws, galois_key, ngalois_elts, galois_elts, param_ctx, nbytes_poly0s, poly0s, nbytes_seeds, seeds);
    CC_FREE_WORKSPACE(ws);
    return rv;
}
...
static CC_WARN_RESULT CC_NONNULL((1)) uint32_t
    ccbfv_galois_key_galois_element(ccbfv_galois_key_const_t galois_key, uint32_t galois_elt_idx)
{
    cc_assert(galois_elt_idx < galois_key->ngalois_elts);
    const uint32_t *galois_elts = CCBFV_GALOIS_KEY_GALOIS_ELTS_CONST(galois_key);
    return galois_elts[galois_elt_idx];
}

bool ccbfv_galois_key_find_galois_elt(uint32_t *galois_elt_idx, uint32_t galois_elt, ccbfv_galois_key_const_t galois_key)
{
    for (uint32_t i = 0; i < galois_key->ngalois_elts; ++i) {
        if (ccbfv_galois_key_galois_element(galois_key, i) == galois_elt) {
            if (galois_elt_idx) {
                *galois_elt_idx = i;
            }
            return true;
...
cc_size CCBFV_CIPHERTEXT_APPLY_GALOIS_WORKSPACE_N(cc_size degree, cc_size nctext_moduli)
{
    return CCBFV_CIPHERTEXT_GALOIS_KEY_SWITCH_WORKSPACE_N(degree, nctext_moduli + 1);
}

int ccbfv_ciphertext_apply_galois_ws(cc_ws_t ws,
...
{
    int rv = CCERR_OK;
    cc_require_or_return(ccbfv_galois_key_find_galois_elt(NULL, galois_elt, galois_key), CCERR_PARAMETER);
    cc_require_or_return(r != ctext, CCERR_PARAMETER);

    for (uint32_t poly_idx = 0; poly_idx < ctext->npolys; ++poly_idx) {
        ccpolyzp_po2cyc_coeff_const_t ctext_poly = ccbfv_ciphertext_coeff_polynomial_const(ctext, poly_idx);
        ccpolyzp_po2cyc_coeff_t r_poly = ccbfv_ciphertext_coeff_polynomial(r, poly_idx);
        rv = ccpolyzp_po2cyc_coeff_apply_galois(r_poly, ctext_poly, galois_elt);
        cc_require(rv == CCERR_OK, errOut);
    }
    rv = ccbfv_ciphertext_galois_key_switch_ws(ws, r, galois_elt, galois_key);

errOut:
    return rv;
}

int ccbfv_ciphertext_apply_galois(ccbfv_ciphertext_coeff_t r,
                                  ccbfv_ciphertext_coeff_const_t ctext,
                                  uint32_t galois_elt,
                                  ccbfv_galois_key_const_t galois_key)
{
    ...
    ccpolyzp_po2cyc_ctx_const_t key_ctx = ccbfv_param_ctx_encrypt_key_context(galois_key->param_ctx);
    CC_DECL_WORKSPACE_OR_FAIL(ws, CCBFV_CIPHERTEXT_APPLY_GALOIS_WORKSPACE_N(key_ctx->dims.degree, key_ctx->dims.nmoduli));
    int rv = ccbfv_ciphertext_apply_galois_ws(ws, r, ctext, galois_elt, galois_key);
    CC_FREE_WORKSPACE(ws);
    return rv;
}

cc_size CCBFV_CIPHERTEXT_GALOIS_KEY_SWITCH_WORKSPACE_N(cc_size degree, cc_size ngalois_key_moduli)
{
    uint32_t npolys = ccbfv_ciphertext_fresh_npolys();
    ...
    return (ccbfv_ciphertext_nof_n(&galois_key_dims, npolys)) + (2 * CCPOLYZP_PO2CYC_NUNITS_PER_COEFF * degree * npolys) +
           (ccpolyzp_po2cyc_nof_n(&buffer_dims)) + (ccpolyzp_po2cyc_ctx_nof_n((uint32_t)degree)) +
           CCBFV_CIPHERTEXT_MOD_SWITCH_DOWN_WORKSPACE_N(CCPOLYZP_PO2CYC_NUNITS_PER_COEFF);
}

int ccbfv_ciphertext_galois_key_switch_ws(cc_ws_t ws,
                                          ccbfv_ciphertext_coeff_t r,
                                          uint32_t galois_elt,
                                          ccbfv_galois_key_const_t galois_key)
{
    uint32_t galois_elt_idx;
    cc_require_or_return(ccbfv_galois_key_find_galois_elt(&galois_elt_idx, galois_elt, galois_key), CCERR_PARAMETER);
    ...
    uint32_t ctext_nmoduli = ccbfv_ciphertext_coeff_ctx(r)->dims.nmoduli;
    ...
    ccbfv_ciphertext_eval_t ctext_galois_prod =
        CCBFV_CIPHERTEXT_EVAL_ALLOC_WS(ws, galois_key_ctx, ccbfv_ciphertext_fresh_npolys());
    ccbfv_ciphertext_eval_init(ctext_galois_prod, galois_key->param_ctx, ccbfv_ciphertext_fresh_npolys(), galois_key_ctx);

    uint32_t degree = galois_key_ctx->dims.degree;
    ccpolyzp_po2cyc_coeff_t ct_galois = ccbfv_ciphertext_coeff_polynomial(r, 1);
    ...
    for (uint32_t rns_idx = 0; rns_idx < galois_key_nmoduli; ++rns_idx) {
        ...
        for (uint32_t j = 0; j < ctext_nmoduli; ++j) {
            ...
            ccbfv_ciphertext_eval_const_t galois_cipher = ccbfv_galois_key_ciphertext_const(galois_key, 0, j);
            ...
            for (uint32_t k = 0; k < ctext_npolys; ++k) {
                ccpolyzp_po2cyc_eval_const_t gk_poly = ccbfv_ciphertext_eval_polynomial_const(galois_cipher, k);
        ...
        for (uint32_t k = 0; k < ctext_npolys; ++k) {
            ccpolyzp_po2cyc_eval_t poly_prod = ccbfv_ciphertext_eval_polynomial(ctext_galois_prod, k);
            for (uint32_t coeff_idx = 0; coeff_idx < degree; ++coeff_idx) {
    ...
    cc_require((rv = ccbfv_ciphertext_inv_ntt(ctext_galois_prod)) == CCERR_OK, errOut);
    ccbfv_ciphertext_coeff_t ctext_galois_prod_coeff = (ccbfv_ciphertext_coeff_t)ctext_galois_prod;
    // Multiply the result by q_{ks}^{-1} mod Q, while switching to base Q
    cc_require((rv = ccbfv_ciphertext_mod_switch_down_ws(ws, ctext_galois_prod_coeff)) == CCERR_OK, errOut);

    // r[0] += ct_galois_prod[0]
    ccpolyzp_po2cyc_coeff_t r_poly_0 = ccbfv_ciphertext_coeff_polynomial(r, 0);
    ccpolyzp_po2cyc_coeff_const_t ctext_galois_prod_poly_0 = ccbfv_ciphertext_coeff_polynomial_const(ctext_galois_prod_coeff, 0);
    ccpolyzp_po2cyc_coeff_add(r_poly_0, r_poly_0, ctext_galois_prod_poly_0);

    // r[1] = ct_galois_prod[1]
    ccpolyzp_po2cyc_coeff_t r_poly_1 = ccbfv_ciphertext_coeff_polynomial(r, 1);
    ccpolyzp_po2cyc_coeff_const_t ctext_galois_prod_poly_1 = ccbfv_ciphertext_coeff_polynomial_const(ctext_galois_prod_coeff, 1);
...
static int ccbfv_galois_elt_from_step(uint32_t *galois_elt, int32_t step, uint32_t degree)
{
    ...
    uint32_t twice_degree = 2 * degree;
    ...
    *galois_elt = 1;
    for (uint32_t i = 0; i < power; ++i) {
        bool overflow = cc_mul_overflow(*galois_elt, ccbfv_encoding_generator_column, galois_elt);
        cc_require_or_return(!overflow, CCERR_INTERNAL);
        *galois_elt &= twice_degree - 1;
    }

    return CCERR_OK;
}

int ccbfv_ciphertext_galois_elt_rotate_rows_left(uint32_t *galois_elt, uint32_t step, uint32_t degree)
{
    CC_ENSURE_DIT_ENABLED

    return ccbfv_galois_elt_from_step(galois_elt, (int32_t)step, degree);
}

int ccbfv_ciphertext_galois_elt_rotate_rows_right(uint32_t *galois_elt, uint32_t step, uint32_t degree)
{
    CC_ENSURE_DIT_ENABLED
...
int ccbfv_ciphertext_galois_elt_swap_columns(uint32_t *galois_elt, uint32_t degree)
{
    CC_ENSURE_DIT_ENABLED

    cc_require_or_return(ccpolyzp_po2cyc_is_power_of_two_uint32(degree), CCERR_PARAMETER);
    *galois_elt = ccbfv_encoding_generator_row(degree);
    return CCERR_OK;
}

cc_size CCBFV_CIPHERTEXT_ROTATE_ROWS_LEFT_WORKSPACE_N(cc_size degree, cc_size nctext_moduli)
{
    return CCBFV_CIPHERTEXT_APPLY_GALOIS_WORKSPACE_N(degree, nctext_moduli);
}

int ccbfv_ciphertext_rotate_rows_left_ws(cc_ws_t ws,
                                         ccbfv_ciphertext_coeff_t r,
                                         ccbfv_ciphertext_coeff_const_t ctext,
                                         uint32_t step,
                                         ccbfv_galois_key_const_t galois_key)
{
    uint32_t degree = ccbfv_param_ctx_polynomial_degree(galois_key->param_ctx);
    cc_require_or_return(step > 0 && step < degree, CCERR_PARAMETER);
    int rv = CCERR_OK;
    uint32_t galois_elt;
    cc_require((rv = ccbfv_ciphertext_galois_elt_rotate_rows_left(&galois_elt, step, degree)) == CCERR_OK, errOut);
    cc_require((rv = ccbfv_ciphertext_apply_galois_ws(ws, r, ctext, galois_elt, galois_key)) == CCERR_OK, errOut);

errOut:
    return rv;
}

cc_size CCBFV_CIPHERTEXT_ROTATE_ROWS_RIGHT_WORKSPACE_N(cc_size degree, cc_size nctext_moduli)
{
    return CCBFV_CIPHERTEXT_ROTATE_ROWS_LEFT_WORKSPACE_N(degree, nctext_moduli);
}

int ccbfv_ciphertext_rotate_rows_right_ws(cc_ws_t ws,
                                          ccbfv_ciphertext_coeff_t r,
                                          ccbfv_ciphertext_coeff_const_t ctext,
                                          uint32_t step,
                                          ccbfv_galois_key_const_t galois_key)
{
    uint32_t degree = ccbfv_param_ctx_polynomial_degree(galois_key->param_ctx);
    cc_require_or_return(step > 0 && step < degree, CCERR_PARAMETER);
    return ccbfv_ciphertext_rotate_rows_left_ws(ws, r, ctext, (degree >> 1) - step, galois_key);
}

cc_size CCBFV_CIPHERTEXT_SWAP_COLUMNS_WORKSPACE_N(cc_size degree, cc_size nctext_moduli)
{
    return CCBFV_CIPHERTEXT_APPLY_GALOIS_WORKSPACE_N(degree, nctext_moduli);
}

int ccbfv_ciphertext_swap_columns_ws(cc_ws_t ws,
                                     ccbfv_ciphertext_coeff_t r,
                                     ccbfv_ciphertext_coeff_const_t ctext,
                                     ccbfv_galois_key_const_t galois_key)
{
    ...
    uint32_t degree = ccbfv_param_ctx_polynomial_degree(galois_key->param_ctx);
    uint32_t galois_elt;
    cc_require((rv = ccbfv_ciphertext_galois_elt_swap_columns(&galois_elt, degree)) == CCERR_OK, errOut);
    cc_require((rv = ccbfv_ciphertext_apply_galois_ws(ws, r, ctext, galois_elt, galois_key)) == CCERR_OK, errOut);
