...
static int ccaes_arm_encrypt_ecb(const ccecb_ctx *key, size_t nblocks, const void *in, void *out)
{
    return AccelerateCrypto_ecb_AES_encrypt((const AccelerateCrypto_AES_ctx *)key, (uint32_t)nblocks, in, out);
}

// Wrapper for ccaes_arm_encrypt_key with key length validation that supports appropriate errors
static int ccaes_arm_encrypt_key_with_key_length_check(const struct ccmode_ecb *ecb CC_UNUSED, ccecb_ctx *key, size_t rawkey_nbytes, const void *rawkey) {
    int rc = ccaes_key_length_validation(rawkey_nbytes);
    cc_require_or_return(rc == CCERR_OK, rc);
    return ccaes_arm_encrypt_key(ecb, key, rawkey_nbytes, rawkey);
}

const struct ccmode_ecb ccaes_arm_ecb_encrypt_mode = {
    .size = sizeof(ccaes_arm_encrypt_ctx),
    .block_size = CCAES_BLOCK_SIZE,
    .init = ccaes_arm_encrypt_key_with_key_length_check,
    .ecb = ccaes_arm_encrypt_ecb,
    .impl = CC_IMPL_AES_ECB_ARM,
};
#else

static int init_wrapper(const struct ccmode_ecb *ecb CC_UNUSED, ccecb_ctx *key, size_t rawkey_nbytes, const void *rawkey)
{
    int rc = ccaes_key_length_validation(rawkey_nbytes);
    cc_require_or_return(rc == CCERR_OK, rc);
    ...
    return ccaes_arm_encrypt_key((const unsigned char *)alignkey, (int)rawkey_nbytes, ctx);
}
...
#if CC_KERNEL
    // CC_ARM_ARCH_7 was scalar code anyway, no need to pack as vector blocks and call the vector code
    if ((((int)in & 0x03) == 0) &&
